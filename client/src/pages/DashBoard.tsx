...
// 1. THIS WAS MISSING IN THE PREVIOUS SNIPPET
interface Note {
    _id: string;
    title: string;
    content: string;
...
const DashBoard = () => {
    ...
    const filteredNotes = notes.filter(note => {
        ...
        // Using optional chaining (?.) here in case tags is undefined on old notes
        const matchesTags = note.tags?.some(tag => tag.toLowerCase().includes(query));
        return matchesTitle || matchesTags;
    });

    return (
        <div className="text-white p-4 md:p-8 w-full max-w-4xl mx-auto overflow-x-hidden">
            {/* Header & Logout */}
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-6">
                ...
                <button 
                    ...
                    className={`font-bold py-2 px-6 rounded transition whitespace-nowrap w-full md:w-auto ${
                        showForm 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredNotes.map((note) => (
                    <div key={note._id} className="bg-[#1F2937] p-5 rounded-lg relative group border border-transparent hover:border-[#4B5563] transition shadow-md">
                        {/* Title: Adjusted padding for mobile to prevent overlap with buttons */}
                        <h3 className="font-bold text-xl text-[#E5E7EB] mb-2 pr-4 md:pr-16 break-words">{note.title}</h3>
                        ...
                        {/* EDIT BUTTONS: Visible on Mobile, Hover only on Desktop */}
